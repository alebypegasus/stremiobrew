...
file_path = "unpacked/usr/palm/services/io.strem.tv.beta.server/beta.html"
with open(file_path, "r", encoding="utf-8") as f:
    html = f.read()

# 1. Add CSS optimizations
css_row = ".row{padding:18px 0 34px 48px;"
css_row_new = ".row{content-visibility:auto;contain-intrinsic-size:100% 450px;padding:18px 0 34px 48px;"
html = html.replace(css_row, css_row_new)

# 2. Add language switcher in Settings
ui_lang_opts = """var UI_LANG_OPTS=[['en','English'],['pt-BR','Português (Brasil)'],['pt-PT','Português (Portugal)'],['es','Español'],['fr','Français'],['de','Deutsch'],['it','Italiano'],['zh','中文'],['ja','日本語'],['ko','한국어'],['hi','हिन्दी'],['ar','العربية'],['ru','Русский'],['tr','Türkçe'],['id','Bahasa Indonesia']];"""
html = html.replace("var LANG_OPTS=", ui_lang_opts + "\\nvar LANG_OPTS=")

settings_defs_orig = "{head:'Subtitles'},"
settings_defs_new = "{head: t('UI Language')},\\n    {k:'uiLang',lb: t('Language'),opts:UI_LANG_OPTS,d:'en'},\\n    {head: t('Subtitles')},"
html = html.replace(settings_defs_orig, settings_defs_new)

# Make the settings reload if uiLang changes
cycle_orig = """lsRawSet(def.k,def.opts[ci][0]);
  if(def.k==='bingeWatch')"""
cycle_new = """lsRawSet(def.k,def.opts[ci][0]);
  if(def.k==='uiLang') setTimeout(function(){location.reload();}, 300);
  if(def.k==='bingeWatch')"""
html = html.replace(cycle_orig, cycle_new)

# 3. Create the translation dictionary and inject it at the top of the <script>
translations = {
  "en": {},
  "pt-BR": {
  ...
  "ko": {
    ...
    "Sign in with your phone": "휴대전화로 로그인", "Scan 단the code with your camera": "카메라로 코드를 스캔하세요",
    ...
    "Add to Library": "보관함에 추가", "In Library": "보관함에 있음", "+ Library": "+ 보관함",
...
js_injection = f"""var TRANSLATIONS = {{}};
// Assign object
...
# 4. Replace hardcoded strings in HTML with JavaScript injected values or dynamic setup
...
# After all HTML, update attributes and content using script
...
# 5. Replace strings in Javascript code
